...
class Decomposer:
    ...
    def decompose(self, filepath, threshold):
        ...
        # remove framework or utility class
        new_class_list = {}
        ...
        for root in to_be_deleted:
            namespace_root = self.findNameSpace(root)
            ...
            namespace_bucket = {}
            for e in final_dict:
                namespace_e = self.findNameSpace(e)
                if namespace_root == namespace_e:
                    for cl in final_dict[e]:
                        if cl not in namespace_bucket:
                            namespace_bucket[cl] = 1
                        else:
                            namespace_bucket[cl] +=1
            
            if len(namespace_bucket) > 0:
                max = -111111111111
                mcl = None
                for el in namespace_bucket:
                    if namespace_bucket[el] > max:
                        max = namespace_bucket[el]
                        mcl = str(el)
                final_dict[root] = [mcl]
            else:
                if namespace_root not in gol_terbuang:
                    gol_terbuang[namespace_root] = []
                gol_terbuang[namespace_root].append(root)
        
        counterr = 1
        for e in gol_terbuang:
            for el in gol_terbuang[e]:
                final_dict[el] = ["99999999999" + str(counterr)]
            counterr +=1
    ...
    def combine(self, final_dict):
        clusters = {}
        for kelas in final_dict:
            for cl in final_dict[kelas]:
                if cl not in clusters:
                    clusters[cl] = []
                clusters[cl].append(kelas)
        while(True):
            lv = 0
            for cluster in clusters:
                kumpulan_anggota = clusters[cluster]
                for anggota in kumpulan_anggota:
                    comp_name = anggota.strip().split("#")
                    end_idx = -1 - lv
                    namespace = "#".join(comp_name[:end_idx])
                    print(namespace)
            break
